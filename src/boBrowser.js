...
async function getSession() {
  ...
  try {
    ...
    if (finalUrl.includes("/login")) {
      // Dump HTML + screenshot để debug
      const html = await page.content().catch(() => "");
      const errorText = html.match(/<[^>]*class="[^"]*error[^"]*"[^>]*>([^<]+)</i)?.[1] || "";
      logger.error("BO login stuck on login page", {
        url: finalUrl,
        errorText,
        htmlSnippet: html.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').slice(0, 300),
      });
      // Thử nhấn Enter thay vì click nút
      await page.keyboard.press("Enter");
      await page.waitForTimeout(3000);
      const urlAfterEnter = page.url();
      if (urlAfterEnter.includes("/login")) {
        throw new Error(`Vẫn ở trang login - URL: ${urlAfterEnter}`);
      }
    }
