...
// ── Public API — Gửi hối thúc hóa đơn về Telegram Admin ──────────────────────
const urgentInvoiceLimit = rateLimit({
  windowMs: 60 * 1000,
  max: 10,
  message: { ok: false, error: "Quá nhiều yêu cầu hối thúc, vui lòng chờ 1 phút" },
  standardHeaders: true,
  legacyHeaders: false,
});

app.post("/api/urgent-invoice", urgentInvoiceLimit, upload.single("image"), async (req, res) => {
  const { username, transferContent, status } = req.body || {};
  const image = req.file;

  if (!username || !transferContent || !image) {
  ...
  if (!token || !chatId) {
    ...
    return res.status(500).json({ ok: false, error: "Chưa cấu hình Telegram Bot Token hoặc Group ID" });
  }

  const safeUsername = String(username).trim().slice(0, 100);
  const safeCK = String(transferContent).trim().slice(0, 100);
  const safeStatus = String(status || "Chưa lên điểm, cần hỗ trợ").trim().slice(0, 150);

  const caption =
    "⚡ YÊU CẦU HỐI THÚC HÓA ĐƠN\n\n" +
    `👤 Tài khoản: ${safeUsername}\n` +
    `🧾 Mã nội dung chuyển khoản: ${safeCK}\n` +
  ...
  try {
    ...
    fd.append("photo", image.buffer, {
      filename: image.originalname || "invoice.jpg",
      contentType: image.mimetype || "image/jpeg",
    });

    const tgRes = await axios.post(`https://api.telegram.org/bot${token}/sendPhoto`, fd, {
    ...
    if (!tgRes.data?.ok) {
      logger.error("Urgent Telegram send failed", { data: tgRes.data });
      return res.status(502).json({ ok: false, error: "Telegram gửi thất bại" });
    }

    logger.info("Urgent invoice sent", { username: safeUsername, ck: safeCK });
    return res.json({ ok: true });
  } catch (err) {
    logger.error("Urgent invoice error", { error: err.message });
    return res.status(500).json({ ok: false, error: "Không gửi được yêu cầu hối thúc" });
  }
});
