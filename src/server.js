...
const { attachDashboardRoutes } = require("./dashboardRoutes");
...
function pushLog(level, msg, meta) {
  ...
  dashboardStore.addLog({ level, msg, meta: meta || {} });
...
app.post("/api/check-invoice", checkInvoiceLimit, upload.single("image"), async (req, res) => {
  ...
  if (transferContent && transferContent.trim().length >= 4) {
    if (ckCheckDuplicate(transferContent, username)) {
      ...
      const duplicateRecord = dashboardStore.addRecord({
        id: Date.now().toString(),
        user: username || "unknown",
        keyword: transferContent || "invoice",
        status: "duplicate",
        value: 0,
        source: "check-invoice",
        note: "Duplicate CK detected",
        createdAt: new Date().toISOString()
      });
      dashboardEmitter.emit("new-record", duplicateRecord);
  ...
  try {
    ...
    const dashboardRecord = dashboardStore.addRecord({
      id: Date.now().toString(),
      user: username || "unknown",
      keyword: transferContent || "invoice",
      status: result?.found ? (result.status || "found") : "not_found",
      value: result?.found ? 1 : 0,
      source: "check-invoice",
      note: result?.note || null,
      createdAt: new Date().toISOString()
    });
    dashboardEmitter.emit("new-record", dashboardRecord);
    ...
    const errorRecord = dashboardStore.addRecord({
      id: Date.now().toString(),
      user: username || "unknown",
      keyword: transferContent || "invoice",
      status: "error",
    ...
    dashboardEmitter.emit("new-record", errorRecord);
...
app.post("/api/urgent-invoice", upload.single("image"), async (req, res) => {
  try {
    ...
    const urgentRecord = dashboardStore.addRecord({
      id: Date.now().toString(),
      user: username || "unknown",
      keyword: transferContent || "urgent-invoice",
      status: "urgent_sent",
      value: 1,
      source: "urgent-invoice",
      note: "Yêu cầu hối thúc hóa đơn từ khách",
      createdAt: new Date().toISOString()
    });
    dashboardEmitter.emit("new-record", urgentRecord);
...
dashboardEmitter.on("new-record", (data) => {
  io.emit("dashboard:new-record", data);
});

dashboardEmitter.on("stats-updated", (data) => {
  io.emit("dashboard:stats", data);
});
