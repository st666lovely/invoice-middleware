...
function ckRegister(ck, username) {
  ...
  const key = ck.toLowerCase().trim();
  if (!ckRegistry.has(key)) ckRegistry.set(key, { username: (username || "").toLowerCase().trim(), time: new Date() });
}

function ckCheckDuplicate(ck, username) {
  const existing = ckRegistry.get(ck.toLowerCase().trim());
  if (!existing) return false;
  return existing.username !== (username || "").toLowerCase().trim();
}
