...
const { fetchOrderInfo } = require("./st666api");
...
app.post("/api/urgent-invoice", upload.single("image"), async (req, res) => {
  try {
    ...
    // ── Tra mã đơn nội bộ từ ST666 ──────────────────────────────────────────────
    let orderCode = "-";
    try {
      const info = await fetchOrderInfo(username);
      if (info?.remarks) {
        orderCode = info.remarks;
        logger.info("ST666 order fetched", { username, orderCode });
      }
    } catch (e) {
      logger.warn("ST666 fetch skipped", { error: e.message });
    }
