...
async function handleCskhCallback(cb) {
  ...
  if (cb.data === "cskh:done") {
    ...
    const sent = await sendTelegramMessage({
      ...
      reply_markup: buildCskhKeyboard(),
    });
...
async function handleCskhNotify(cb) {
  ...
  const caption = [
    ...
    `🔖 ${root.ck_code || "-"}`,
    `📊 ${root.status || "-"}`,
    root.note ? `💬 ${root.note}` : null,
  ].filter(Boolean).join("\n");

  if (root.file_id) {
    await sendTelegramPhoto({ chat_id: notifyGroupId, photo: root.file_id, caption });
  } else {
    await sendTelegramMessage({ chat_id: notifyGroupId, text: caption });
  }
