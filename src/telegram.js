...
function pickT3ChatId(orderCode) {
  ...
  if (!code) return T3_GROUP_ID;

  // Ưu tiên match prefix dài nhất để hỗ trợ mã dạng SG30T74NBD không có dấu phân cách.
  const matchedPrefix = Object.keys(T3_PREFIX_MAP)
    .map(k => String(k).trim().toUpperCase())
    .filter(Boolean)
    .sort((a, b) => b.length - a.length)
    .find(prefix => code.startsWith(prefix));

  return matchedPrefix ? T3_PREFIX_MAP[matchedPrefix] : T3_GROUP_ID;
}
